...
const Sidebar = () => {
  const { user, fetchUser, axios } = useAppContext(); // Fonksiyon çağrıldı
  const location = useLocation();
  const [image, setImage] = useState(null);

  const updateImage = async () => {
    if (!image) return; // image yoksa işlemi durdur
    try {
      ...
      if (data.success) {
        ...
        setImage(null); // state sıfırlanıyor
      } else {
          ...
          <img
            src={
              image
                ...
                : user?.image || 'https://unsplash.com/s/photos/people'
            }
            alt={user?.name || 'User'}
